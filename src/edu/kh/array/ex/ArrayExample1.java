...
public class ArrayExample1 {
	...
	public void ex1() {
		...
		//변수 선언
		int num;
		// Stack 영역에 int 자료형을 저장 할 수 있는 공간 (4byte)를 할당하고
		// 그 공간에 num이라는 이름을 부여한 것.
		
		//변수 대입
		num = 10;
		// 생성된 num이라는 변수 공간 안에 10을 대입한 것.
		...
		// 그 공간에 arr 이라는 이름을 부여한 것.
		// 해당 변수는 참조형으로 주소값만을 저장 할 수 있다.
		
		// 배열의 할당
		arr = new int[3];
		
		// new : "new 연산자" 라고 하며 Heap 메모리 영역에 새로운 공간(배열, 객체)을 할당한다.
		...
		System.out.println( arr[0]); // arr 이 참조하고 있는 배열의 0번째 인덱스 값을 얻어옴
		System.out.println( arr[1]); // arr 이 참조하고 있는 배열의 1번째 인덱스 값을 얻어옴
		System.out.println( arr[2]); // arr 이 참조하고 있는 배열의 2번째 인덱스 값을 얻어옴
...
//		int min = score[0];
//		
//		// 아래 for 문을 이용하여 score 배열에 있는 모든값과 max, min 을 비교
...
//	
	public void ex5() {
		...
		// Arrays.toString(배열명) : 모든 요소값을 출력해줌
	...
	public void ex7() {
		...
		for(int i = 0; i < arr.length; i++) {
			...
			if(arr[i] == input) {
				...
				System.out.println(i + "번째 인텍스에 존재");
				
				flag = true; // 일치하는 값이 있으므로 true 로 변경
	...
	public void ex9() {
		...
		// 2. 문자 하나를 입력 받아 일치하는 문자가 char 매열에 몇개 존재하는지 저장
		...
		Scanner sc = new Scanner(System.in);
			...
			String str = sc.nextLine(); // (한줄 띄어쓰기 포함) 입력받기
			...
			int count = 0; // 같은 글자 개수를 세기위한 변수
			
			
			for(int i =0; i<arr.length;i++) {
				if(arr[i] == ch) {
					//arr[i] 값과 검색할 문자 ch 가 같은 경우 카운트
